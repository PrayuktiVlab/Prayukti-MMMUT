...
import { ChevronRight, PlayCircle, ArrowLeft, Award, CheckCircle2, Lock } from "lucide-react";
...
export default function CPage() {
    const labs = useMemo(() => getLabsBySubject("C"), []);
    ...
    useEffect(() => {
        ...
        const subjectProgress = getSubjectProgress("c");
        const labIds = labs.map(p => p.id);

        setStudentNameStr(studentName);
        setStudentRollStr(studentRoll);
        setProgress(subjectProgress);
        setOverallCompletion(getSubjectCompletionRate("c", labIds));
        setIsFullyCompleted(isSubjectCompleted("c", labIds));
    }, [labs]);
    ...
    const handleGenerateFinalCertificate = async () => {
        ...
        await generateCertificate(studentNameStr, "C Programming Lab", true, studentRollStr);
    };
        ...
        <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white pb-10">
            <header className="bg-white border-b-2 border-black/5 sticky top-0 z-20 backdrop-blur-md bg-white/80">
                <div className="container mx-auto px-4 py-4 flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold uppercase tracking-wider text-gray-400">Dashboard</span>
            ...
            <main className="container mx-auto px-4 py-12 max-w-5xl">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
                    <div>
                        <h2 className="text-4xl lg:text-5xl font-black uppercase tracking-tighter mb-2 text-green-600">Experiments</h2>
                        <p className="text-lg text-gray-500 font-medium">Master the foundations of C programming.</p>
                    </div>
                </div>
                        ...
                        return (
                            <div key={p.id} className="group flex flex-col md:flex-row md:items-center justify-between p-6 border-b-2 border-black/5 hover:bg-gray-50 transition-colors gap-6">
                                    ...
                                    <div className="space-y-3 w-full max-w-xl">
                                        <div>
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <span className="text-xs font-bold text-gray-400 tracking-wider uppercase">EST. TIME: {p.metadata.estimatedTime || "30 MIN"}</span>
                                            </div>
                                ...
                                <div className="flex items-center gap-3 shrink-0">
                                    <Link href={`/dashboard/c/${p.id}`} className="w-full md:w-auto">
