...
export default function DSALabDetail({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    const labId = id.startsWith('dsa-exp-') ? id : `dsa-exp-${id}`;
        ...
        <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white">
            <header className="bg-white border-b-2 border-black/5 sticky top-0 z-20 backdrop-blur-md bg-white/80">
                <div className="container mx-auto px-4 py-4 flex items-center gap-4">
                    ...
                    <span className="text-sm font-bold uppercase tracking-wider text-gray-400">DSA</span>
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            <Badge variant="outline" className={`rounded-none uppercase text-[10px] font-bold tracking-widest border-2 ${lab.metadata.difficulty === 'Easy' ? 'border-green-600 text-green-700 bg-green-50' :
                                lab.metadata.difficulty === 'Medium' ? 'border-purple-600 text-purple-700 bg-purple-50' :
                ...
                <div className="lg:col-span-4 space-y-8">
                    <div className="sticky top-24 space-y-8">
                        <div className="bg-purple-50 border-2 border-purple-100 rounded-xl p-8 relative overflow-hidden group">
                            ...
                            <h3 className="font-black text-xl uppercase tracking-tight mb-2 relative z-10 text-purple-900">Ready to Analyze?</h3>
                            <p className="text-sm font-medium text-purple-800/70 mb-8 relative z-10 leading-relaxed">
                                Launch the algorithmic sandbox to visualize and test your implementation.
                            </p>
                            <Link href={`/dashboard/dsa/${labId}/simulation`}>
            ...
            <Chatbot subject="DSA" labTitle={lab.metadata.title} />
            <Footer />
        </div>
    );
}
