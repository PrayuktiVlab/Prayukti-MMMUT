...
export default function SimulationPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    const labId = id.startsWith('oops-exp-') ? id : `oops-exp-${id}`;
        ...
        <div className="flex flex-col h-screen bg-gray-100">
            <header className="bg-white border-b shadow-sm px-4 py-3 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-bold text-gray-800">OOPS Simulation Workbench <span className="text-gray-400 text-sm font-normal">| Practical {id}</span></h1>
                </div>
            </header>
